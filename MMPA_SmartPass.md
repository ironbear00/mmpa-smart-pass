...
| Dynamic Document Guide | `Verdict.requiredDocs`를 UI에 렌더 | 불필요 서류 안내 제거 |
...
export interface Verdict {
  ...
  loffId?: string;
  listGrade?: "L1" | "L2" | "L3";
  documentTrack?: "catch_certificate" | "coa"; // 수입산 eligible 시 결정
...
| `DOMESTIC_DOCS` / `CATCH_CERT_DOCS` / `COA_STANDARD_DOCS` / `COA_L3_EXEMPTION_DOCS` | TS 상수 (트랙별 분리) | `src/rules/required-docs.ts` |
...
```
[국가]          → conditional, listGrade, missing: [itemCd]
[+ 품목]        → eligible / ineligible, listGrade, requiredDocs
```

등급 판정과 적격 판정은 분리된다. **L2 국가도 품목 조건 충족 시 eligible.** 배지 색상은 등급 표시일 뿐 적격성이 아니다.

가공국은 `COA_ITEM_LIST.defaultProcessingCountry`로 품목별 고정 매핑되며 UI에 노출하지 않는다. 판정 로직(`verifyCoa`)은 원산지·품목만 사용한다. (→ D-008)
...
| COA `listGrade: "L1"` | 녹색 Badge |
| COA `listGrade: "L2"` | 황색 Badge |
| COA `listGrade: "L3"` | 적색 Badge + 추가 서류 안내 |
...
│   ├── api/                # Hono 서버
│   │   ├── server.ts
│   │   ├── repository.ts   # DB 교체 인터페이스 (→ D-007)
│   │   ├── db.ts           # SQLite 구현체 (repo: Repository export)
│   │   └── routes/
│   └── ui/                 # Vite + React
│       ├── main.tsx
